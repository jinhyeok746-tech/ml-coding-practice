...
# 시청 등급 기준표를 참고하여 Netfilx의 rating 변수를 이용한 age_group_dic 변수 생성
# rating 컬럼의 값을 age_group 이라는 새로운 컬럼으로 복사
...
# title열의 값이 'Sankfa'인 행 전체를 확인하여 country 열과 age_group 열의 값이 어떻게 이루어져 있는지 확인
...
plt.suptitle('Target ages proportion of total contatnt by country',
...
# mask : 단어를 그릴 위치 설정, 흰색(#FFFFF) 항목은 마스킹된 것으로 간주
...
wordcloud = WordCloud(background_color='white', mask=mask, colormap=cmap).generate(text) 